...
class Brander_Opcheckout_Block_Adminhtml_Sales_Order_View_Opcheckoutcomment extends Mage_Adminhtml_Block_Sales_Order_View_Items {
            ...
            $html .= '<div id="customer_comment" class="giftmessage-whole-order-container">
                        <div class="entry-edit">
                            <div class="entry-edit-head">
                                <h4>' . $this->helper('brander_opcheckout')->__('Customer Comment') . '</h4>
                            </div>
                            <fieldset>' . nl2br($this->helper('brander_opcheckout')->htmlEscape($comment)) . '</fieldset>
                        </div>
                      </div>';
        }
        return $html;
    ...
    public function getDeliveryDateHtml() {
        ...
        if (Mage::getStoreConfigFlag('opcheckout/order/opcheckout_order_deliverydate') == 1 && $ddate != '') {
            $html .= '<div id="delivery_date" class="delivery-whole-order-container">
                        <div class="entry-edit">
                            <div class="entry-edit-head">
                                <h4>' . $this->helper('brander_opcheckout')->__('Deliver Date') . '</h4>
                            </div>
                            <fieldset>' . nl2br($this->helper('brander_opcheckout')->htmlEscape($formateddate)) . '</fieldset>
                        </div>
                      </div>';
        }
        return $html;
