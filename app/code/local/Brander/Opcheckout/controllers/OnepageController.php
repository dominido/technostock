...
class Brander_Opcheckout_OnepageController extends Mage_Checkout_OnepageController {
    ...
    public function getOnepage() {
        return Mage::getSingleton('brander_opcheckout/type_onepage');
    }
    ...
    public function saveShippingMethodAction() {
        ...
        if ($this->getRequest()->isPost()) {
             ...
             */
            if (!$result) {
                Mage::dispatchEvent('opcheckout_save_shipping_method', array('request' => $this->getRequest(), 'quote' => $this->getOnepage()->getQuote()));
                $this->getResponse()->setBody(Zend_Json::encode($result));
            ...
            try {
                ...
                if (is_array($billingRes) && isset($billingRes['error'])) {
                    ...
                    if(is_array($billingRes['message'])) {
                        ...
                        foreach ($billingRes['message'] as $mess) {
                            $result['error_messages'][] = Mage::helper('checkout')->__($mess);
                        }
                    } else {
                ...
                if (!$checkout->getQuote()->isVirtual()) {
                    ...
                    if (is_array($shippingRes) && isset($shippingRes['error'])) {
                        ...
                        if(is_array($shippingRes['message'])) {
                            $result['error_messages'] = array();
                            foreach ($shippingRes['message'] as $mess) {
                                $result['error_messages'][] = Mage::helper('checkout')->__($mess);
                            }
                        } else {
                    ...
                    Mage::dispatchEvent('opcheckout_controller_onepage_before_save_shipping_method', array('request' => $this->getRequest(), 'quote' => $this->getOnepage()->getQuote()));
                    $shippingMethodRes = $checkout->saveShippingMethod($shippingMethod);
                    if (is_array($shippingMethodRes) && isset($shippingMethodRes['error'])) {
                        $result['success'] = false;
                        $result['error'] = true;

                        if(is_array($shippingMethodRes['message'])) {
                            $result['error_messages'] = array();
                            foreach ($shippingMethodRes['message'] as $mess) {
                                $result['error_messages'][] = Mage::helper('checkout')->__($mess);
                            }
                        } else {
                ...
                if (is_array($paymentRes) && isset($paymentRes['error'])) {
                    ...
                    if(is_array($paymentRes['message'])) {
                        $result['error_messages'] = array();
                        foreach ($paymentRes['message'] as $mess) {
                            $result['error_messages'][] = Mage::helper('checkout')->__($mess);
                        }
                    } else {
...
//            newsletter subscription
    ...
    public function removeQuoteItemAction() {
        ...
        if($quoteItemId) {
            ...
            if (!$quote->hasItems()) {
                $response['redirect'] = Mage::app()->getStore()->getUrl('checkout/cart');
            }
            ...
            $quote->save();
            $response['success'] = true;
            $response['message'] = $this->__('Item remove successfully.');
        } else {
            $response['error'] = true;
            $response['message'] = $this->__('Item remove error.');
        }

        $response['cartBar'] = $this->getLayout()->createBlock('checkout/cart_sidebar')->setTemplate('checkout/cart/mini.phtml')->toHtml();

        $this->getResponse()->setBody(Zend_Json::encode($response));
    }

    public function changeQuoteItemQtyAction() {
        $response = array(
            'success' => false,
            'error' => false,
            'message' => false,
        ...
        $quoteItemId = $this->getRequest()->getParam('item_id');
        $quoteItemQty = $this->getRequest()->getParam('item_qty');

        if($quoteItemId && $quoteItemQty) {
            $quote          = $this->_getOnepage()->getQuote();
            $item           = $quote->getItemById($quoteItemId);
            $_product = Mage::getModel('catalog/product')->load($item->getProductId());

            if ($_product->getStockItem()->getManageStock()) {
                $maximumAllowQty = intval($_product->getStockItem()->getMaxSaleQty());

                if ($quoteItemQty > $maximumAllowQty) {
                    $response['error'] = true;
                    $response['message'] = $this->__('Maximum Allowed Qty: %s', round($maximumAllowQty));
                }
                ...
                if ($item->getHasChildren()) {
                    foreach ($item->getChildren() as $child) {
                        ...
                        if ($quoteItemQty > $maximumQty) {
                            $response['error'] = true;
                            $response['message'] = $this->__('Maximum Allowed Qty: %s', round($maximumQty));
                        }
                    }
                } else {
                ...
                $minimumAllowQty = intval($_product->getStockItem()->getMinSaleQty());
                if ($quoteItemQty < $minimumAllowQty) {
                    $response['error'] = true;
                    $response['message'] = $this->__('Minimal Allowed Qty: %s', round($minimumAllowQty));
                }
            ...
            if(!$response['error'] && !$response['message']) {
                ...
                $quote->save();
                $response['success'] = true;
                $response['message'] = $this->__('Item qty was changed successfully.');
            }
        } else {
            $response['error'] = true;
            $response['message'] = $this->__('Item qty changed has error.');
        }
    ...
    protected function _canShowForUnregisteredUsers() {
        return Mage::helper('brander_opcheckout')->isActive();
    }
}
