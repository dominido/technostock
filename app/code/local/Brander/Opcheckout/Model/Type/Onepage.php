...
class Brander_Opcheckout_Model_Type_Onepage extends Mage_Checkout_Model_Type_Onepage {
    ...
    protected function _processValidateCustomer(Mage_Sales_Model_Quote_Address $address) {
        ...
        if (Mage_Sales_Model_Quote::CHECKOUT_METHOD_REGISTER == $this->getQuote()->getCheckoutMethod()) {
            ...
            foreach ($fieldsArray as $key => $dataKey) {
    ...
    public function saveOrder() {
        ...
        if (!$this->getQuote()->isVirtual()) {
			$orderShipping = $convertQuote->addressToOrderAddress($shipping);
            $order->setShippingAddress($orderShipping);
            $street = $orderShipping->getStreet();
            if (is_array($street) && isset($street[0])) {
                $order->setEmailStreet($street[0]);
            }
			$order->setEmailCity($orderShipping->getCity());
        }
