 ...
 */
class Brander_Cms_Model_Resource_Eav_Mysql4_Page_Tree extends Varien_Data_Tree_Dbp
{
    /**
    ...
    public function __construct()
    {
        ...
        parent::__construct(
            ...
            $resource->getTableName('cmsadvanced/page'),
            array(
    ...
    protected function _getIsActiveAttributeId()
    {
        if (is_null($this->_isActiveAttributeId)) {
            $select = $this->_conn->select()
                ...
                ->where('entity_type_code = ?', 'cms_page')
    ...
    protected function _getInactiveItemIds($collection, $storeId)
    {
        ...
        $table = Mage::getSingleton('core/resource')->getTableName('cmsadvanced/page') . '_int';
        $select = $this->_conn->select()
    ...
    protected function _getDefaultCollection($sorted=false)
    {
        ...
        $collection->addAttributeToSelect(array('name', 'is_active'));
     ...
     * @return Mage_Catalog_Model_Resource_Eav_Mysql4_Category_Tree
     */
    protected function _beforeMove($page, $newParent, $prevNode)
    {
        Mage::dispatchEvent('cmsadvanced_page_tree_move_before',
            array(
                'page' => $page,
    ...
    public function move($page, $newParent, $prevNode = null) {
        ...
        $this->_beforeMove($page, $newParent, $prevNode);
        Mage::getResourceSingleton('cmsadvanced/page')->move($page->getId(), $newParent->getId());
    ...
    protected function _afterMove($page, $newParent, $prevNode)
    {
        ...
        Mage::dispatchEvent('cmsadvanced_page_tree_move_after',
            array(
                'page' => $page,
                'prev_node' => $prevNode,
                'parent' => $newParent
    ...
    protected function _createCollectionDataSelect($sorted = true, $optionalAttributes = array())
    {
        ...
        foreach ($attributes as $attributeCode) {
            $attribute = Mage::getResourceSingleton('cmsadvanced/page')->getAttribute($attributeCode);
        ...
        /*$pagesTable         = Mage::getSingleton('core/resource')->getTableName('catalog/category');
        ...
        $select->joinLeft(array('_category_product' => $categoriesProductsTable),
            ...
            array(
                ...
                'product_count' => new Zend_Db_Expr('(SELECT COUNT(DISTINCT cp.product_id) FROM ' . $pagesTable . ' ee
