...
class Brander_AutoImport_Block_Adminhtml_Stages_Stageone extends Mage_Adminhtml_Block_Widget_Grid_Container
{
    public function __construct()
    {
        $this->_blockGroup = 'autoimport';
        $this->_controller = 'adminhtml_stages_stageone';
        $stageFromDate = Mage::helper('autoimport/stages')->getStagePeriod(2);
        $stageToDate = Mage::helper('autoimport/stages')->getStagePeriod(1);
        parent::__construct();
        $this->_headerText = Mage::helper('autoimport')->__('STAGE 1. Out of stock products, update from %s to %s', $stageFromDate, $stageToDate);
