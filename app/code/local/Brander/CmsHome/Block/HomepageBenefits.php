 ...
 */
class Brander_CmsHome_Block_HomepageBenefits extends Mage_Core_Block_Template
{
    ...
    {
        if(!$this->_collection){
            $this->_collection = Mage::getModel('brander_benefits/benefit')
                ->getCollection()
                ->setStoreId(Mage::app()->getStore()->getId())
                ->addAttributeToSelect('*')
                ->addAttributeToFilter('status', 1);
            $this->_collection->getSelect()->order('related_product.position', 'ASC');
        }
        return $this->_collection;
    }
