...
class Brander_LayeredNavigation_Controller_Router extends Mage_Core_Controller_Varien_Router_Abstract
{
    const MIDDLE    = 0;
    const BEGINNING = 1;
    ...
    public function match(Zend_Controller_Request_Http $request)
    {
        ...
        $this->urlMode = Mage::getStoreConfig('brander_layerednavigation/seo/urls');

        $isMatched = false;

        if (!$isMatched && $this->urlMode == Brander_LayeredNavigation_Model_Source_Url_Mode::MODE_SHORT) {
    ...
    protected function matchForwardShort()
    {
        ...
        /** @var Brander_LayeredNavigation_Model_Url_Parser $shortParser */
        $shortParser = Mage::getModel('brander_layerednavigation/url_parser');
        ...
        if (strlen($cat)){ // normal category
            ...
            if (!Mage::getStoreConfig('brander_layerednavigation/seo/enable_layerednavigation_page')) {
    ...
    protected function matchForwardMultilevel()
    {
        ...
        if ($this->urlMode == Brander_LayeredNavigation_Model_Source_Url_Mode::MODE_SHORT && Mage::getStoreConfig('brander_layerednavigation/seo/redirects_enabled')) {
    ...
    {
        $reservedKey = Mage::getStoreConfig('brander_layerednavigation/seo/key') . '/';
        ...
        if ($params || $params === '0'){
            Mage::register('brander_layerednavigation_current_params', $params);
    ...
    {
        $code = Mage::getStoreConfig('brander_layerednavigation/brands/attr');
        $code = trim(str_replace('_', Mage::getStoreConfig('brander_layerednavigation/seo/special_char'), $code));
        ...
        foreach ($options[$code] as $key => $id) {
            if (!Mage::getStoreConfig('brander_layerednavigation/seo/hide_attributes')){
                $key = $code . Mage::getStoreConfig('brander_layerednavigation/seo/option_char') . $key;
    ...
    protected function forwardCategory($cat)
    {
        ...
        Mage::register('brander_layerednavigation_forwarded_category_id', $categoryId);
    }

    protected function forwardShopby()
    {
        $reservedKey = Mage::getStoreConfig('brander_layerednavigation/seo/key');
        $realModule = 'Brander_LayeredNavigation';

        $this->request->setPathInfo($reservedKey);
        $this->request->setModuleName('brander_layerednavigation');
        $this->request->setRouteName('brander_layerednavigation');
