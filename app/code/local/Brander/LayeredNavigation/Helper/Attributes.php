...
class Brander_LayeredNavigation_Helper_Attributes extends Brander_LayeredNavigation_Helper_Cached
{
    protected $_options;
    ...
    public function getAllFilterableOptionsAsHash()
    {
        ...
        $requireUniqueOptions = Mage::getStoreConfig('brander_layerednavigation/seo/hide_attributes') || Mage::getStoreConfig('brander_layerednavigation/seo/urls') == Brander_LayeredNavigation_Model_Source_Url_Mode::MODE_SHORT;
        ...
        foreach ($attributes as $a){
            ...
            $code = str_replace(array('_', '-'), Mage::getStoreConfig('brander_layerednavigation/seo/special_char'), $code);
            $hash[$code] = array();
            foreach ($options as $o){
                if (($o['value'] || $o['value'] === '0') && $o['attribute_id'] == $a->getId()) { // skip first empty
                    ...
                    while (isset($hash[$code][$unKey])
                        ...
                        $unKey .= Mage::getStoreConfig('brander_layerednavigation/seo/special_char');
                    }
    ...
    public function createKey($optionLabel)
    {
        ...
        $key = preg_replace('/[^0-9a-z,]+/i', Mage::getStoreConfig('brander_layerednavigation/seo/special_char'), $key);
        $key = strtolower($key);
        $key = trim($key, Mage::getStoreConfig('brander_layerednavigation/seo/special_char') . Mage::getStoreConfig('brander_layerednavigation/seo/option_char'));

        if ($key == '') {
            $key = Mage::getStoreConfig('brander_layerednavigation/seo/special_char');
        }
    ...
    protected function getAllOptions()
    {
        ...
        $select->joinLeft(
            array('s' => $valuesCollection->getTable('brander_layerednavigation/value')),
    ...
    {
        $brandAttributeCode = trim(Mage::getStoreConfig('brander_layerednavigation/brands/attr'));
        $query = Mage::app()->getRequest()->getQuery();
        if (!isset($query[$brandAttributeCode])) {
            return null;
        }

        $value = Mage::getModel('brander_layerednavigation/value')->load($query[$brandAttributeCode], 'option_id');
        if (!$value->getId()) {
            return null;
        }
