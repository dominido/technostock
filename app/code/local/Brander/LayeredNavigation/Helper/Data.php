...
class Brander_LayeredNavigation_Helper_Data extends Brander_LayeredNavigation_Helper_Cached
{
    protected $_useSolr;
    
    const XML_PATH_SEO_PRICE_NOFOLLOW       = 'brander_layerednavigation/seo/price_nofollow';
    const XML_PATH_SEO_PRICE_NOINDEX        = 'brander_layerednavigation/seo/price_noindex';
    const XML_PATH_SEO_PRICE_RELNOFOLLOW    = 'brander_layerednavigation/seo/price_rel_nofollow';
    ...
    /**
     * @return Brander_LayeredNavigation_Model_Filter[]
     */
    public function getAttributesSettings()
    {
        ...
        $attrCollection = Mage::getResourceModel('brander_layerednavigation/filter_collection')->load();
     ...
     * @param Brander_LayeredNavigation_Block_Catalog_Layer_Filter_Attribute $filter
     *          - used in app/design/frontend/base/default/template/brander/layerednavigation/view_top.phtml
     */
    public function getAttributePosition($filter)
    {
        if( is_object($filter->getAttributeModel()) ) {
            ...
            if (array_key_exists($attributeId, $settings)) {
                return "layerednavigation-filter-position-" .$settings[$attributeId]->getBlockPos();
            }
        }

        if( $filter instanceof Mage_Catalog_Block_Layer_Filter_Price ){
            return "layerednavigation-filter-position-" . Mage::getStoreConfig('brander_layerednavigation/block/price_pos');
        }

        if( $filter instanceof Brander_LayeredNavigation_Block_Catalog_Layer_Filter_Stock ){
            return "layerednavigation-filter-position-" . Mage::getStoreConfig('brander_layerednavigation/block/stock_filter_pos');
        }

        if( $filter instanceof Mage_Catalog_Block_Layer_Filter_Category ){
            return "layerednavigation-filter-position-" . Mage::getStoreConfig('brander_layerednavigation/block/categories_pos');
        }

        if( $filter instanceof Brander_LayeredNavigation_Block_Catalog_Layer_Filter_Rating ){
            return "layerednavigation-filter-position-" . Mage::getStoreConfig('brander_layerednavigation/block/rating_filter_pos');
        }
    ...
    public function getIconsData()
    {
        ...
        $optionCollection = Mage::getResourceModel('brander_layerednavigation/value_collection')
            ->addPositions()
            ->addFieldToFilter('img_medium', array('gt' => ''))  
            ->addValue();  

        $result = array();
        /** @var Brander_LayeredNavigation_Helper_Url $hlp */
        $hlp = Mage::helper('brander_layerednavigation/url');
        foreach ($optionCollection as $opt){
            ...
            $result[$opt->getOptionId()] = array(
                ...
                'img'   => Mage::getBaseUrl('media') . 'brander/layerednavigation/' . $img,
    ...
    public function showLinks($product, $mode='view', $names=array(), $exclude=false)
    {
        ...
        usort($items, array('Brander_LayeredNavigation_Helper_Data', '_srt'));
        
        //create block
        $block = Mage::getModel('core/layout')->createBlock('core/template')
            ->setArea('frontend')
            ->setTemplate('brander/layerednavigation/links.phtml');
    ...
    protected function getMultiselectAttributeCodes()
    {
        ...
        /** @var Brander_LayeredNavigation_Helper_Attributes $attributesHelper */
        $attributesHelper = Mage::helper('brander_layerednavigation/attributes');
    ...
    public function getCurrentCategory()
    {
        ...
        $forwardedCategoryId = Mage::registry('brander_layerednavigation_forwarded_category_id');
