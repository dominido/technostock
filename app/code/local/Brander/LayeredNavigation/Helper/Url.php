...
class Brander_LayeredNavigation_Helper_Url extends Mage_Core_Helper_Abstract
{
    ...
    {
        $key = Mage::getStoreConfig('brander_layerednavigation/seo/key');
        $category = $this->_getCurrentCategory();
        $canonicalType = Mage::getStoreConfig('brander_layerednavigation/seo/canonical' . (is_object($category) ? '_cat' : ''));

        switch ($canonicalType) {
            case Brander_LayeredNavigation_Model_Source_Canonical::CANONICAL_KEY:
                return $category ? $category->getUrl() : (Mage::getBaseUrl() . $key);

            case Brander_LayeredNavigation_Model_Source_Canonical::CANONICAL_CURRENT_URL:
                return $this->_getCurrentUrlWithoutParams();

            case Brander_LayeredNavigation_Model_Source_Canonical::CANONICAL_FIRST_ATTRIBUTE_VALUE:
    ...
    protected function _getFirstAttributeValueUrl()
    {
        ...
        /** @var Brander_LayeredNavigation_Helper_Data $dataHelper */
        $dataHelper = Mage::helper('brander_layerednavigation');
    ...
    /**
     * @deprecated Now it is a facade to Brander_LayeredNavigation_Model_Url
     */
    public function getFullUrl($query=array(), $clear=false, $cat = null)
    {
        /** @var Brander_LayeredNavigation_Model_Url_Builder $builder */
        $builder = Mage::getModel('brander_layerednavigation/url_builder');
        $builder->reset();
    ...
    public function saveParams($request)
    {
        ...
        $currentParams = Mage::registry('brander_layerednavigation_current_params');
        if (!$currentParams){
            return true;
        }

        // brand-amd-canon/price-100,200 or  amd-canon/100,200
        $hideAttributeNames = Mage::getStoreConfig('brander_layerednavigation/seo/hide_attributes');

        foreach ($currentParams as $params){

            $attrCode = '';

            $params   = explode(Mage::getStoreConfig('brander_layerednavigation/seo/option_char'), $params);
            $firstOpt = $params[0];
            ...
            if ($attrCode && isset($options[$attrCode])){
                ...
                if ($this->isDecimal($attrCode)){
                    ...
                    $v = $params[0];
                    if (count($params) > 1){
                        $v = $params[0] . Mage::getStoreConfig('brander_layerednavigation/seo/option_char') . $params[1];
                    }
    ...
    public function isBrandPage($cat, $params)
    {
        ...
        $attrCode = trim(Mage::getStoreConfig('brander_layerednavigation/brands/attr'));
        if (!$attrCode) {
            return false;
        }
    ...
    {
        return Mage::helper('brander_layerednavigation/attributes')->getAllFilterableOptionsAsHash();
    }
    ...
    private function _convertIdToKeys($options, $ids)
    {
        ...
        return join(Mage::getStoreConfig('brander_layerednavigation/seo/option_char'), $keys);
    }

    public function _formatAttributePartMultilevel($attrCode, $ids)
    {
        if ($this->isDecimal($attrCode)){
            return $attrCode . Mage::getStoreConfig('brander_layerednavigation/seo/option_char') . $ids; // always show price and other decimal attributes
        }
        ...
        $hideAttributeNames = Mage::getStoreConfig('brander_layerednavigation/seo/hide_attributes');
        $part =  $hideAttributeNames ? $part : ($attrCode . Mage::getStoreConfig('brander_layerednavigation/seo/option_char') . $part);
    ...
    {
        if ($this->isDecimal($attrCode)){
            return $attrCode . Mage::getStoreConfig('brander_layerednavigation/seo/option_char') . $ids; // always show other decimal attributes
        }
    ...
    {
        return str_replace(array(Mage::getStoreConfig('brander_layerednavigation/seo/option_char'), Mage::getStoreConfig('brander_layerednavigation/seo/special_char')), '_', $attrCode);
    }
