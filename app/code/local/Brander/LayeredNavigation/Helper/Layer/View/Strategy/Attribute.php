...
class Brander_LayeredNavigation_Helper_Layer_View_Strategy_Attribute extends Brander_LayeredNavigation_Helper_Layer_View_Strategy_Modeled
{
    ...
    {
        $template = 'brander/layerednavigation/attribute.phtml';

        $isSwatchesDisplayType = is_object($this->model) && $this->model->getDisplayType() == Brander_LayeredNavigation_Model_Source_Attribute::DT_MAGENTO_SWATCHES;
        if ($isSwatchesDisplayType) {
            if ($this->isSwatchesAvailable()) {
                $template = 'configurableswatches/catalog/layer/filter/swatches.phtml';
            } else {
                $this->model->setDisplayType(Brander_LayeredNavigation_Model_Source_Attribute::DT_LABELS_ONLY);
            }
        }
    ...
    {
        if (Mage::helper('brander_layerednavigation')->isModuleEnabled('Mage_ConfigurableSwatches')) {
    ...
    protected function prepareItems()
    {
        ...
        foreach ($items as $item){
            /** @var Brander_LayeredNavigation_Model_Catalog_Layer_Filter_Item $item */
    ...
    public function getIsExcluded()
    {
        ...
        // hide when selected
        //$hideBySingleChoice = (defined('BRANDER_FEATURE_HIDE_SINGLE_CHOICE_FILTERS') && BRANDER_FEATURE_HIDE_SINGLE_CHOICE_FILTERS && $this->model->getSingleChoice());
        $hideBySingleChoice = $this->model->getSingleChoice();
        $hideByConfigurableSwatches = false;
        //$hideByConfigurableSwatches = $this->model->getDisplayType() == Brander_LayeredNavigation_Model_Source_Attribute::DT_MAGENTO_SWATCHES && $this->isSwatchesAvailable();
