...
class Brander_LayeredNavigation_Adminhtml_PageController extends Mage_Adminhtml_Controller_Action
{
    public function indexAction()
    {
        $this->loadLayout(); 
        $this->_setActiveMenu('catalog/layerednavigation/pages');
        $this->_addContent($this->getLayout()->createBlock('brander_layerednavigation/adminhtml_page'));
        $this->renderLayout();
    }
    ...
    public function editAction() 
    {
        ...
        $model  = Mage::getModel('brander_layerednavigation/page')->load($id);
        ...
        if ($id && !$model->getId()) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('brander_layerednavigation')->__('Page does not exist'));
            $this->_redirect('*/*/');
            return;
        }
        ...
        Mage::register('brander_layerednavigation_page', $model);
        
        $this->loadLayout();
        $this->_setActiveMenu('catalog/layerednavigation');
        $this->_title('Edit Page');
        $this->_addContent($this->getLayout()->createBlock('brander_layerednavigation/adminhtml_page_edit'));
    ...
    public function saveAction() 
    {
        ...
        $model  = Mage::getModel('brander_layerednavigation/page');
        ...
        if ($data) {
            ...
            try {
                ...
                $msg = Mage::helper('brander_layerednavigation')->__('Page has been successfully saved');
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('brander_layerednavigation')->__('Unable to find a page to save'));
        $this->_redirect('*/*/');
    } 
    ...
    public function massDeleteAction()
    {
        ...
        if(!is_array($ids)) {
             Mage::getSingleton('adminhtml/session')->addError(Mage::helper('brander_layerednavigation')->__('Please select page(s)'));
        } 
        else {
            try {
                foreach ($ids as $id) {
    ...
    public function deleteAction() 
    {
        if ($this->getRequest()->getParam('id') > 0 ) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('brander_layerednavigation')->__('Page has been deleted'));
                $this->_redirect('*/*/');
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
