...
class Brander_LayeredNavigation_Model_Url_Builder
{
    /** @var  string */
    public $moduleName;
    ...
    public function reset()
    {
        ...
        $this->mode = Mage::getStoreConfig('brander_layerednavigation/seo/urls');

        // Configuration parameters
        $this->brandAttributeCode = Mage::getStoreConfig('brander_layerednavigation/brands/attr');
    }
    ...
    {
        $this->moduleName = 'brander_layerednavigation';
    }
    ...
    {
        $excludeParamsStr = trim(Mage::getStoreConfig('brander_layerednavigation/seo/query_param_exclude'));
        if ($excludeParamsStr != '') {
    ...
    {
        $paramName = Mage::getStoreConfig('brander_layerednavigation/seo/query_param');
        if ($paramName) {
    ...
    protected function getParamPart()
    {
        ...
        if ($this->mode && !$this->isSomeSearch()) {
            ...
            {
                $attrCode = str_replace(array('_', '-'), Mage::getStoreConfig('brander_layerednavigation/seo/special_char'), $origAttrCode);

                if (isset($options[$attrCode])){ // it is filterable attribute
                    if ($this->mode == Brander_LayeredNavigation_Model_Source_Url_Mode::MODE_SHORT) {
        ...
        $glue = ($this->mode == Brander_LayeredNavigation_Model_Source_Url_Mode::MODE_SHORT) ? Mage::getStoreConfig('brander_layerednavigation/seo/option_char') : '/';
    ...
    protected function getBasePart($paramPart)
    {
        ...
        elseif ($this->moduleName == 'cms' && $this->getCategoryId() == $rootId) { // homepage,
            $hasFilter = false;
            if (Mage::getStoreConfig('brander_layerednavigation/block/ajax')) {
                $hasFilter = true;
            }
            if (!$hasFilter) {
        ...
        elseif ($this->getCategoryId() == $rootId) {
            ...
            switch ($this->mode) {
                case Brander_LayeredNavigation_Model_Source_Url_Mode::MODE_DISABLED:
                    $needUrlKey = true;
                    break;
                case Brander_LayeredNavigation_Model_Source_Url_Mode::MODE_MULTILEVEL:
                    $needUrlKey = !$this->isBrandPage();
                    break;
                case Brander_LayeredNavigation_Model_Source_Url_Mode::MODE_SHORT:
    ...
    protected function isBrandPage()
    {
        ...
        $isBrandPage = $this->moduleName == 'brander_layerednavigation' && $isAttributeRequested;
        return $isBrandPage;
    }
    ...
    {
        return $this->mode == Brander_LayeredNavigation_Model_Source_Url_Mode::MODE_MULTILEVEL || $this->mode == Brander_LayeredNavigation_Model_Source_Url_Mode::MODE_DISABLED;
    }
}
