...
/**
 * Class Brander_LayeredNavigation_Model_Catalog_Layer_Filter_Attribute
 * @method Mage_Catalog_Model_Resource_Eav_Attribute getAttributeModel()
 */
class Brander_LayeredNavigation_Model_Catalog_Layer_Filter_Attribute extends Mage_Catalog_Model_Layer_Filter_Attribute
{
    ...
    public function applyFilterToCollection($value) {
        ...
        if (Mage::helper('brander_layerednavigation')->useSolr()) {
    ...
    {
        $currentVals = Mage::helper('brander_layerednavigation')->getRequestValues($this->_requestVar);
        if ($currentVals) {
            ...
            $branding = $controller == 'Brander_LayeredNavigation'
                && count($currentVals) == 1
                && trim(Mage::getStoreConfig('brander_layerednavigation/brands/attr')) == $attributeCode;
            if (!$branding) {
    ...
    protected function addState($currentVals)
    {
        ...
        $attribute = $this->getAttributeModel();
        $text = '';
        $options = Mage::helper('brander_layerednavigation/attributes')->getAttributeOptions($attribute->getAttributeCode());
        ...
        foreach ($options as $option) {
            ...
            if (false !== $k){
                ...
                $url = Mage::helper('brander_layerednavigation/url')->getFullUrl($query);
        ...
        /** @var Brander_LayeredNavigation_Model_Catalog_Layer_Filter_Item $state */
    ...
    protected function getSingleChoice()
    {
        ...
        return ($settings->getDisplayType() == Brander_LayeredNavigation_Model_Source_Attribute::DT_DROPDOWN) ? true : $settings->getSingleChoice();
    }
