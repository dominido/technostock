...
class Brander_LayeredNavigation_Model_Catalog_Layer_Filter_Stock extends Mage_Catalog_Model_Layer_Filter_Abstract
{
    ...
    public function apply(Zend_Controller_Request_Abstract $request, $filterBlock)
    {
        ...
        $state = $this->_createItem($filter == self::FILTER_IN_STOCK ? Mage::helper('brander_layerednavigation')->__('In Stock') : Mage::helper('brander_layerednavigation')->__('Out of Stock'), $filter)
    ...
    {
        return Mage::helper('brander_layerednavigation')->__('Stock Filter');
    }
    ...
    protected function _getItemsData()
    {
    	...
    	$data[] = array(
        	'label' => Mage::helper('brander_layerednavigation')->__('In Stock'),
            'value' => ($currentValue == self::FILTER_IN_STOCK) ? null : self:: FILTER_IN_STOCK,
            'count' => $in_stock[0],
            'option_id' => self:: FILTER_IN_STOCK,
		);
		$data[] = array(
        	'label' => Mage::helper('brander_layerednavigation')->__('Out of Stock'),
