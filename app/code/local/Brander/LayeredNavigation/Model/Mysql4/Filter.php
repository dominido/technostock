...
class Brander_LayeredNavigation_Model_Mysql4_Filter extends Mage_Core_Model_Mysql4_Abstract
{
    public function _construct()
    {    
        $this->_init('brander_layerednavigation/filter', 'filter_id');
    }
    ...
    public function getIdByCode($code)
    {
        ...
        $sql = $db->select()
            ->from(array('f' => $this->getTable('brander_layerednavigation/filter')), array('f.filter_id'))
    ...
    public function getFilterByAttributeId($id)
    {
        ...
        $sql = $db->select()
            ->from(array('f' => $this->getTable('brander_layerednavigation/filter')), array('f.*'))
    ...
    public function createFilters()
    {
        try {
            ...
            $sqlIds = (string)$db->select()
                    ->from(array('f' => $this->getTable('brander_layerednavigation/filter')), array('filter_id'));
            $o = $this->getTable('brander_layerednavigation/value');
            ...
            $sql = $db->select()
                ...
                ->joinLeft(array('f' => $this->getTable('brander_layerednavigation/filter')), 'a.attribute_id = f.attribute_id', array())
            ...
            foreach ($res as $row) {
                ...
                if ($row['attribute_code'] != 'price') {
                    $db->insert($this->getTable('brander_layerednavigation/filter'), $data);
                }
            }
            ...
            $sql = $db->select()
                ...
                ->joinInner(array('f' => $this->getTable('brander_layerednavigation/filter')), 'o.attribute_id = f.attribute_id', array('f.filter_id'))
                ->joinInner(array('ov' => $this->getTable('eav/attribute_option_value')), 'o.option_id = ov.option_id', array('ov.option_id'))
                ->joinLeft(array('v' => $this->getTable('brander_layerednavigation/value')), 'v.option_id = o.option_id', array())
                ->where('ov.store_id = 0')
                ->where('v.value_id IS NULL');

            $insertSql = 'INSERT INTO ' . $this->getTable('brander_layerednavigation/value') . '(filter_id, option_id) ' .  (string)$sql;
            ...
            $msg = Mage::helper('brander_layerednavigation')->__('Improved Navigation filters and their options have been loaded');
    ...
    protected function AddMultistoreValues($arrIds){
        ...
        if ($res) {
            ...
            foreach ($options as $key => $value) {
                $ser = serialize($value);
                $db->update($this->getTable('brander_layerednavigation/value'), array('title' => $ser, 'meta_title' => $ser), "option_id=" . $key);
            }
        }
    }
}