...
class Brander_LayeredNavigation_Block_Advanced extends Mage_Catalog_Block_Navigation
{
    /** @var  Brander_LayeredNavigation_Model_Url_Builder */
    protected $urlBuilder;
    protected $_advancedCategories = array();
    ...
    public function drawOpenCategoryItem($category, $level = 0)
    {
        ...
        $showAll   = Mage::getStoreConfig('brander_layerednavigation/advanced_categories/show_all_categories');
        $showDepth = Mage::getStoreConfig('brander_layerednavigation/advanced_categories/show_all_categories_depth');
    ...
    {
        if (!$this->_advancedCategories) {
            $categoryModel = Mage::getSingleton('brander_layerednavigation/catalog_layer_filter_category');

            $this->_advancedCategories = $categoryModel->getAdvancedCollection();
        }

        return $this->_advancedCategories;
    }
    ...
    {
        return Mage::getStoreConfigFlag('brander_layerednavigation/advanced_categories/display_product_count');
    }
    ...
    protected function _isExcluded($categoryId)
    {
        if (!$this->hasData('exclude_ids')) {
            $excludeIds = preg_replace('/[^\d,]+/', '', Mage::getStoreConfig('brander_layerednavigation/general/exclude_cat'));
        ...
        if (!$this->hasData('include_ids')) {
            $includeIds = preg_replace('/[^\d,]+/', '', Mage::getStoreConfig('brander_layerednavigation/general/include_cat'));
