...
class Brander_LayeredNavigation_Block_Adminhtml_Filter_Edit_Tab_General extends Mage_Adminhtml_Block_Widget_Form
{
    /**
     * Filter form
    ...
    /** @var  Brander_LayeredNavigation_Model_Filter */
    protected $model;
    ...
    protected function _prepareForm()
    {
        ...
        $this->model = Mage::registry('brander_layerednavigation_filter');
    ...
    protected function _prepareRegularForm()
    {
        ...
        $fldSet3 = $this->_form->addFieldset('brander_layerednavigation_seo', array('legend'=> $this->__('Search Engines Optimization')));
    ...
    {
        $fldSet = $this->_form->addFieldset('brander_layerednavigation_general', array('legend'=> $this->__('Display Properties')));

        $fldSet->addField('block_pos', 'select', array(
            'label'     => $this->__('Show in the Block'),
            'name'      => 'block_pos',
            'values'    => Mage::getModel('brander_layerednavigation/source_position')->toOptionArray(),
        ));
    ...
    {
        $fldSet2 = $this->_form->addFieldset('brander_layerednavigation_blocks', array('legend'=> $this->__('Additional Blocks')));
        ...
        $fldSet2->addField('show_on_view', 'select', array(
            ...
            'name'      => 'show_on_view',
            'values'    => $this->yesno,
            'note'      => $this->__('Show options images block on product view page. You need to perform modifications in theme template, see Layered Navigation User Guide on page 15.'),
        ));
    }

    protected function _prepareFieldsetSpecial()
    {
        $fldSet = $this->_form->addFieldset('brander_layerednavigation_special', array('legend'=> $this->__('Special Cases')));
    ...
    protected function setupDependencies()
    {
        ...
        if ($this->isDecimal()) {
            $mapper
                ...
                ->addFieldDependence('slider_type', 'display_type', Brander_LayeredNavigation_Model_Catalog_Layer_Filter_Price::DT_SLIDER)
                ->addFieldDependence('slider_decimal', 'display_type', Brander_LayeredNavigation_Model_Catalog_Layer_Filter_Price::DT_SLIDER)
                ;
        } else {
            $mapper
