...
class Brander_LayeredNavigation_Block_List extends Mage_Core_Block_Template
{
    private $items = array();
    ...
    protected function _prepareLayout()
    {
        ...
        $images = Mage::getStoreConfig('brander_layerednavigation/brands/show_images') ? $this->_getOptionImages($options) : null;
    ...
    protected function _getOptionImages($options)
    {
        ...
        $collection = Mage::getResourceModel('brander_layerednavigation/value_collection')
            ->addFieldToFilter('option_id', array('in'=>$ids))
            ->load();
        $images = array();
        foreach ($collection as $value){
            $images[$value->getOptionId()] = $value->getImgBig() ? Mage::getBaseUrl('media') . 'layerednavigation/' . $value->getImgBig() : null;
        }
        return $images;
    }
