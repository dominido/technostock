...
class Brander_LayeredNavigation_Block_Top extends Mage_Core_Block_Template
{
    ...
    {
        return 'brander_layerednavigation-filters-wrapper';
    }
    ...
    {
        /** @var Brander_LayeredNavigation_Model_Mysql4_Page $pageResource */
        $pageResource = Mage::getResourceModel('brander_layerednavigation/page');
        ...
        $titleSeparator = Mage::getStoreConfig('brander_layerednavigation/general/title_separator');
        $descrSeparator = Mage::getStoreConfig('brander_layerednavigation/general/descr_separator');
        $kwSeparator = ',';
    ...
    protected function _prepareLayout()
    {
        ...
        /** @var Brander_LayeredNavigation_Block_Catalog_Product_List_Toolbar $toolbar */
        $toolbar = $this->getLayout()->getBlock('product_list_toolbar');
        if ($toolbar instanceof Brander_LayeredNavigation_Block_Catalog_Product_List_Toolbar) {
        ...
        $filters = Mage::getResourceModel('brander_layerednavigation/filter_collection')
                ->addTitles()
                ->setOrder('position');
        $hash = array();
        ...
        foreach ($filters as $f){
            /** @var Brander_LayeredNavigation_Model_Filter $f */
        ...
        $catNoIndex = Mage::getStoreConfig('brander_layerednavigation/seo/cat_noindex');
        if ($catNoIndex != '') {
        ...
        $catNoFollow = Mage::getStoreConfig('brander_layerednavigation/seo/cat_nofollow');
        if ($catNoFollow != '') {
        ...
        $options = Mage::getResourceModel('brander_layerednavigation/value_collection')
        ...
        foreach ($options as $opt){
            /** @var Brander_LayeredNavigation_Model_Value $opt */
            $id = $opt->getOptionId();
    ...
    protected function addBrandBreadcrumb()
    {
        ...
        if ($brandPageBrand) {
            ...
            $breadcrumbs->addCrumb('brander_layerednavigation-brand', array('label' => $brandPageBrand->getCurrentTitle(), 'title' => $brandPageBrand->getCurrentTitle()));
        }
    }
    ...
    protected function changeCategoryData(Mage_Catalog_Model_Category $category)
    {
        ...
        if ($brandPageBrand) {
            ...
            $category->setData('image', $brandPageBrand->getImgBig() ? '../../layerednavigation/' .$brandPageBrand->getImgBig() : null);
        ...
        foreach ($this->options as $opt){
            ...
            if ($opt->getImgBig()){
                $imageUrl = '../../brander_layerednavigation/' . $opt->getImgBig();
            }
        }

        $position = Mage::getStoreConfig('brander_layerednavigation/heading/add_title');
        $title = $this->insertContent($category->getName(), $titles, $position, Mage::getStoreConfig('brander_layerednavigation/heading/h1_separator'));
        $category->setData('name', $title);

		if($category->getData('h1_tag')) {
			$title = $this->insertContent($category->getData('h1_tag'), $titles, $position, Mage::getStoreConfig('brander_layerednavigation/heading/h1_separator'));
			$category->setData('name', $title);
			$category->setData('h1_tag', $title);
		}

        $position = Mage::getStoreConfig('brander_layerednavigation/heading/add_description');
        if ($descriptions && $position != Brander_LayeredNavigation_Model_Source_Description_Position::DO_NOT_ADD) {
            $oldDescription = $category->getData('description');
            $description = '<span class="layerednavigation-descr">' . join('<br>', $descriptions) . '</span>';
            switch ($position) {
                case Brander_LayeredNavigation_Model_Source_Description_Position::AFTER:
                    $description = $oldDescription ? $oldDescription . '<br>' . $description : $description;
                    break;
                case Brander_LayeredNavigation_Model_Source_Description_Position::BEFORE:
                    $description = $oldDescription ? $description . '<br>' . $oldDescription : $description;
                    break;
                case Brander_LayeredNavigation_Model_Source_Description_Position::REPLACE:
                    break;
            }
            $category->setData('description', $description);
        }

        if (isset($imageUrl) && Mage::getStoreConfig('brander_layerednavigation/heading/add_image')) {
            $category->setData('image', $imageUrl);
        }

        if (isset($cmsBlockId) && Mage::getStoreConfig('brander_layerednavigation/heading/add_cms_block')) {
    ...
    protected function changeMetaData($head)
    {
        ...
        $titlePosition = Mage::getStoreConfig('brander_layerednavigation/meta/add_title');
        $titleSeparator = Mage::getStoreConfig('brander_layerednavigation/meta/title_separator');
        ...
        $descriptionPosition = Mage::getStoreConfig('brander_layerednavigation/meta/add_description');
        $descrSeparator = Mage::getStoreConfig('brander_layerednavigation/meta/descr_separator');
        ...
        $keywordsPosition = Mage::getStoreConfig('brander_layerednavigation/meta/add_keyword');
    ...
    /**
     * @return Brander_LayeredNavigation_Model_Value|null
     */
    protected function getCurrentBrandPageBrand()
    {
        ...
        /** @var Brander_LayeredNavigation_Helper_Attributes $helper */
        $helper = Mage::helper('brander_layerednavigation/attributes');
    ...
    {
        if ($newParts && $position != Brander_LayeredNavigation_Model_Source_Description_Position::DO_NOT_ADD) {
            if ($original) {
                switch ($position) {
                    case Brander_LayeredNavigation_Model_Source_Description_Position::AFTER:
    ...
    public function handleExtraAttributes()
    {
        ...
        if ($head){
            ...
            if (Mage::app()->getRequest()->getParam('price')) {
                $robotsIndex = Mage::getStoreConfig('brander_layerednavigation/general/price_tag_noindex');
                $robotsFollow = Mage::getStoreConfig('brander_layerednavigation/general/price_tag_nofollow');
