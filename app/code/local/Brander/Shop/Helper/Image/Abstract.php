 ...
 */
abstract class Brander_Shop_Helper_Image_Abstract extends Mage_Core_Helper_Data
{
    /**
     ...
     * @return Brander_Shop_Helper_Image_Abstract
    ...
    public function init(Varien_Object $object, $imageField = 'image')
    {
        ...
        try {
            if ($this->getImageType($this->_image) == 'svg') {
                if (is_file($this->getImageBaseDir().$this->_image)) {
    ...
    protected function _getImageProcessor()
    {
        if (is_null($this->_imageProcessor)) {
            ...
            $this->_imageProcessor->backgroundColor($this->getBackgroundColor());
        }
     ...
     * @access public
     * @param bool $value
     * @return mixed(bool|Brander_Shop_Helper_Image_Abstract)
    ...
    protected function _getDestinationImagePrefix()
    {
        ...
        if ($this->_scheduledResize) {
            ...
            if ($keepFrame) {
                $imageRealPath .= '-frame';
                $_backgroundColor = $this->getBackgroundColor();
                if ($_backgroundColor) {
