<?php
class Brander_SearchAutocomplete_Model_Mysql4_Fulltext extends Mage_Core_Model_Mysql4_Abstract {
    const XML_PATH_FILTER_CMS_PAGES = 'brander_autocomplete/searchautocomplete/filter_cms_pages';
        ...
        try {
            ...
            while (true) {
                ...
                foreach ($pages as $pageData) {
                    //load widgets
                    $pageData['content'] = preg_replace_callback('@{{.*?}}@si','Brander_SearchAutocomplete_Helper_Data::loadWidget', $pageData['content']);
            ...
            foreach ($news as $newsData) {
                ...
                if (isset($newsData['summary'])) {
                    $html = "";
                    $newsData['summary'] = preg_replace_callback('@{{.*?}}@si','Brander_SearchAutocomplete_Helper_Data::loadWidget', $newsData['summary']);
                    try {
                ...
                if (isset($newsData['content'])) {
                    $html = "";
                    $newsData['content'] = preg_replace_callback('@{{.*?}}@si','Brander_SearchAutocomplete_Helper_Data::loadWidget', $newsData['content']);
                    try {
